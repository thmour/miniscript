...
function evaluate(t, env) {
    ...
    switch (t.type) {
        ...
        case 'condition':
            return evaluate(t.condition, env) + '?' + evaluate(t.left, env) + ':' + evaluate(t.right, env);
        ...
        case 'object':
            ...
            while (--i) {
                result = ',' + evaluate(t.value[i].left, env) + ':' + evaluate(t.value[i].right, env) + result;
            }

            result = evaluate(t.value[0].left, env) + ':' + evaluate(t.value[0].right, env) + result;
        ...
        case 'import':
            ...
            if(/\.mini/.test(t.module)) {
                ...
                return 'var ' + t.alias + ' = require(' + t.module + ')' + sector + ';';
            }
