...
function evaluate(t, env) {
    ...
    switch (t.token) {
        ...
        case 'for':
			var assign_str = '';
			if(t.iterator_list.length == 1) {
				assign_str = t.iterator_list[0] + '= _c[_i];';
			}
			else {
				_.each(t.iterator_list, function(v,i) {
					assign_str += v + '= c[_i][' + i + '];';
				})
			}
            t.body.value.unshift(assign_str);
            return 'for(var _i = -1, _c = ' + evaluate(t.collection,env) + ', _l = _c.length,' + t.iterator_list + '; ++_i<_l;)' + body_eval(t.body,env);
        case 'forin':
            return '_c = ' + evaluate(t.collection,env) + ';for(var ' + evaluate(t.iterator,env) + ' in _c)' + body_eval(t.body,env);
