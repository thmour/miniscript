...
function evaluate(t, env) {
    ...
    switch (t.type) {
        ...
        case 'literal':
            var access = '';
            for (var i = -1, l = t.accessor.length; ++i < l;) {
                access += access_eval(t.accessor[i], env);
            }
            return (+t.value && t.accessor[0] ? '(' + t.value + ')' : evaluate(t.value, env)) + access;
        ...
        case 'access':
            return evaluate(t.left, env) + access_eval(t.right, env);
        ...
        case 'new':
            ...
            return 'new ' + t.name + access + (access && t.accessor[t.accessor.length - 1].type == 'call' ? '' : '()');
        ...
        case 'class':
            ...
            if (t.parent) {
                if (t.body.value[0].type == 'super') {
                    ...
                    final_str += t.parent + '.apply(this, [].slice.call(arguments, 0, ' + classes[t.parent].args.length + '));';
                }
            }
        ...
        case 'paren':
            ...
            return '(' + evaluate(t.value, env) + ')' + access;
