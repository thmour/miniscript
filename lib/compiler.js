        ...
        case 'for':
            ...
            if (t.op == 'in') {
                if (t.iterators.length == 1) {
                    ...
                    var_lookup('_c', env);
                    col = evaluate(t.collection, env);
                    return '_c = ' + col + ';for(' + t.iterators[0] + ' in _c)' + body_eval(t.body, env);
                } else {
                    _.each(t.iterators, function (v) {
                        var_lookup(v, env);
                        assign_str += v + '= _c[_i].' + v + ';';
                    });
                    var_lookup('_i', env);
                    var_lookup('_c', env);
                    col = evaluate(t.collection, env);
                    t.body.value.unshift(assign_str);
                    return '_c = ' + col + ';for(_i = -1, _l = _c.length; ++_i<_l;)' + body_eval(t.body, env);
                }
            } else {
                if (t.iterators.length == 1) {
                    var_lookup(t.iterators[0], env);
                    assign_str += t.iterators[0] + '= _c[_i];';
                } else {
                    _.each(t.iterators, function (v, i) {
                        var_lookup(v, env);
                        assign_str += v + '= _c[_i][' + i + '];';
                    })
                }
                var_lookup('_i', env);
                var_lookup('_c', env);
